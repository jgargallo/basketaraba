...
refresh the local data and preview the exact same static output locally, run:
...
data/<group-slug>/
  ...
  matches/<partido_id>.json        per-match: quarters, both team boxes, full log  ← committed to git
  raw/                             download cache (HTML)                            ← gitignored
    calendario.html
...
|---|---|---|
| `scrape.yml` | Daily at 06:00 UTC + manual | Runs the full pipeline, commits updated `matches/` and `database.json` back to `main` |
| `pages.yml` | Push to `main` | Builds `web/dist/` from `database.json` and deploys to GitHub Pages |

The scrape commit triggers the Pages deploy automatically — no manual step needed.

**What is committed to git:**

| Path | Committed | Reason |
|---|---|---|
| `data/*/matches/*.json` | yes | canonical scraped data; CI skips already-scraped matches |
| `data/*/matches.json` | yes | season index |
| `data/*/database.json` | yes | derived tables; Pages deploy needs no scraping |
| `data/*/raw/*.html` | **no** | download cache only; ephemeral in CI |
| `web/dist/` | **no** | built by `pages.yml` on every deploy |

To run the full pipeline manually (e.g. to backfill data locally), trigger
`scrape.yml` from the GitHub Actions UI with **Run workflow**.
